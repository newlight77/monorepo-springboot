...
import io.oneprofile.signup.metafile.Representation
import io.oneprofile.signup.metafile.fromModel
import io.oneprofile.signup.metafile.toMetafile
...
class SignupWebHandler(val signupService: ISignupService,
    ...
    private var backendBaseUrl = env.getProperty("signup.baseUrl")!!
    ...
    fun addComment(targetUsername: String, comment: CommentModel): CommentModel {
        val newComment = this.signupService.addComment(targetUsername, fromModel(comment))
        logger.info("successfully verified the activation code for user $targetUsername")
