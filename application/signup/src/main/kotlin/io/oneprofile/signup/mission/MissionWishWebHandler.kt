...
import io.oneprofile.signup.exception.GlobalNotAcceptedException
...
class MissionWishWebHandler(val missionWishService: IMissionWishService,
    ...
    fun update(missionWish: MissionWishModel): MissionWishModel {
        val domain = try {
            ...
            logger.error("Failed to update the missionWish for user ${missionWish.username}", ex)
    ...
    fun updateResume(username: String, file: MultipartFile): MissionWishModel {
        ...
        val result = try {
            ...
            logger.error("Failed to upload the mission specific resume for user $username", ex)
    ...
    fun updateOnResumeUploaded(username: String, filename: String) {
        try {
            ...
            logger.error("Failed to update mission wish on resume upload for user $username", ex)
