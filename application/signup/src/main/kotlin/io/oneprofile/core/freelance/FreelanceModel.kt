...
import io.oneprofile.core.company.AddressModel
import io.oneprofile.core.company.CompanyModel
import io.oneprofile.core.company.ContactModel
import io.oneprofile.core.company.PrivacyDetailModel
...
fun toModel(domain: FreelanceDomain): FreelanceModel {
    return FreelanceModel.Builder(domain.username)
            .contact(domain.contact?.let { io.oneprofile.core.company.toModel(it) })
            .address(domain.address?.let { io.oneprofile.core.company.toModel(it) })
            .company(domain.company?.let { io.oneprofile.core.company.toModel(it) })
            .privacyDetail(domain.privacyDetail?.let { io.oneprofile.core.company.toModel(it) })
...
fun fromModel(model: FreelanceModel): FreelanceDomain {
    return FreelanceDomain.Builder(model.username)
            .contact(model.contact?.let { io.oneprofile.core.company.fromModel(it) })
            .address(model.address?.let { io.oneprofile.core.company.fromModel(it) })
            .company(model.company?.let { io.oneprofile.core.company.fromModel(it) })
            .privacyDetail(model.privacyDetail?.let { io.oneprofile.core.company.fromModel(it) })
