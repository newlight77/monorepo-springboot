...
class SignupEventPublisher(private val applicationEventPublisher: ApplicationEventPublisher) {
    ...
    fun publishEmailNotification(username: String, notification: EmailNotificationDomain) {
        try {
            applicationEventPublisher.publishEvent(
                NotificationEvent(username, notification)
            )
            logger.info("A SignupEmailNotificationEvent has been published for user $username")
        } catch (ex: Exception) {
            logger.error("Failed to publish a SignupEmailNotificationEvent for user $username")
            throw EmailNotifiicationPublicationException("Failed to publish a SignupEmailNotificationEvent for user $username")
        }
    }
